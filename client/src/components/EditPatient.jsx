...
const EditPatient = ({ patientId, patient: initialPatient, onSuccess }) => {
  ...
  useEffect(() => {
    // Use id from URL params, or patientId prop
    ...
    if (initialPatient) {
      setForm({
        ...
        contactNumber: initialPatient.contactNumber || '',
      ...
      const fetchPatient = async () => {
        try {
          const res = await fetch(`${backend}/api/patient/${currentId}`);
  ...
  }, [initialPatient, patientId, id, backend]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const currentId = (initialPatient && initialPatient._id) || id || patientId;
      if (!currentId) {
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="col-span-1 md:col-span-2 flex flex-col">
            ...
            <textarea
              ...
              onChange={handleChange}
              className="border border-gray-300 rounded px-3 py-2 h-28 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-transparent"
              style={{ color: 'var(--app-text)' }}
            />
          </div>
          ...
          <div className="col-span-1 md:col-span-2">
            <button
              type="submit"
              className="w-full rounded-full font-bold py-2 px-4 shadow card transition-all mt-2" style={{background:'var(--accent)', color:'#fff'}} onMouseEnter={e=>e.currentTarget.style.background='var(--accent-hover)'} onMouseLeave={e=>e.currentTarget.style.background='var(--accent)'}>Update</button>
          </div>
        </form>
      </div>
    </div>
